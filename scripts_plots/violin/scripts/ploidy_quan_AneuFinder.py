...
def chr_extract(chr):
	if chr[-1]=="X" or chr[-1]=="x" or chr[-1] == "chrX":
		return 23
	elif chr[-1]=="Y" or chr[-1]=="y" or chr[-1] == "chrY":
		return 24
	else:
		chrString = ""
		for i in chr:
        				...
        				if len(GS_array[Chr])>0:
        					...
        					if len(Ginkgo_array[Chr])>0:
        						...
        						for Tbp in GS_array[Chr]:
        							...
        							for Bp in Ginkgo_array[Chr]:
        								if abs(Tbp.pos-Bp.pos)<Thr and Tbp.cn==Bp.cn:
        									if found_flag:
        										FP_ginkgo+=1
        									else:
        										TP_ginkgo+=1
        										found_flag=True
        									Bp.set_flag(1)
        								elif abs(Tbp.pos-Bp.pos)<Thr and Tbp.cn!=Bp.cn:
        									FP_ginkgo+=1
        									Bp.set_flag(1)
...
if(len(sys.argv) <= 1):
	...
	Usage: python """ + sys.argv[0] + """ <experiment> <rep:1-3> <num_p>
	""")
	sys.exit(0)
exp = sys.argv[1]
rep = sys.argv[2]
...
for met in range(len(methods)):
        ...
        for i in range(num_ploidy):
                met_col = np.repeat(met, len(Recall[i]))
...
g = g.map(sns.violinplot, "fluc", "value", "method", cut=0, inner=None, scale="width", split=False, palette="muted", saturation=1).despine(left=True)
# Set axis labels & ticks #
fontsize=12
fontsize1=14
fontsize2=16
g.fig.get_axes()[0].set_xticklabels([1.5, 2.1, 3.0, 3.8, 5.3], size=fontsize)
g.fig.get_axes()[1].set_xticklabels([1.5, 2.1, 3.0, 3.8, 5.3], size=fontsize)
...
#g.fig.get_axes()[1].legend([handles[0], handles[1]], ["Ginkgo", "HMMcopy"], loc='upper right', fontsize='x-large')
# Fixing titles #
g.fig.get_axes()[0].set_title("")
g.fig.get_axes()[1].set_title("")
#plt.show()
fig_file = "../figs/" + exp + "/combined_quan_rep" + str(rep) + ".png"
plt.tight_layout()
